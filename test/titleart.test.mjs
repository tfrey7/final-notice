...
test('the back layer keeps to 8 BG palettes of 15 colours, one a tile, the crown on its own', () => {
  const { back, glow, w, h } = TITLE_ART;
  assert.ok(back.palettes.length <= 8);
  assert.ok(back.palettes.every((p) => p.length === 15 && channels(p)));
  assert.equal(back.tiles.length, (w / 8) * (h / 8));
  assert.ok([...back.tiles].every((t) => parseInt(t, 16) < back.palettes.length));
  assert.ok(glow.length > 0 && glow.every((p) => p < back.palettes.length));
  assert.ok(hexRows(back.pixels, w, h));
});

test('the partners keep to at most 4 sprite palettes and are clear outside their tiles', () => {
  const { front, w, h } = TITLE_ART;
  assert.ok(front.palettes.length >= 1 && front.palettes.length <= 4);
