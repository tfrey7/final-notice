...
| Sprites | 128 OAM entries on screen; each entry 8x8, 16x16, 32x32 or 64x64 (one size pair per screen, this game uses **16x16 and 32x32**). At most **32 entries on one scanline** and **34 8x8 tiles' worth** (time over); past that the lowest-priority entries vanish — the SNES drops, it does not flicker (**back:** no NES rotation flicker). |
...
| Sound | SPC700 + S-DSP: 8 voices of BRR sample playback (4-bit, 16:9 compressed), 32 kHz output, Gaussian interpolation (the soft, warm top end), per-voice ADSR or GAIN, pitch modulation, one noise source, an echo with an 8-tap FIR filter; about 60 KB of sample memory for the whole game's instruments, so samples are short loops. A sound effect takes a voice (voice 8, then 7) from the music and gives it back. |
...
From memory, unverified; the research card (`docs/SNES-CLASSICS.md`) checks each and adds sources.

| Game | What it did | Final Notice takes |
| --- | --- | --- |
| Final Fight (Capcom, 1990) | Huge, readable characters (Haggar near 100 px), heavy hit-stop, few foes on screen at once, belt floors with deep perspective | Stage 1 scale and 2-3 foes; hit-stop in the tuning table; a one-colour floor gradient under HDMA |
| Sunset Riders (Konami, 1993) | Cartoon clarity, bosses introduced by name with a wanted poster, speech-bubble taunts, sampled voice | a boss title card (name + department, a filed-memo look) and one sampled voice line per boss |
| TMNT IV: Turtles in Time (Konami, 1992) | Throwing foes into the screen (Mode 7 scaling), crowded-but-fluid brawling, remixed arcade music with sampled drums, time-travel palette variety | the "throw into the camera" finisher on the last foe of an area; palette-swapped foes for variety from few assets |
| Super Castlevania IV (Konami, 1991) | Mode 7 rotating rooms and swinging chandeliers used once or twice as spectacle, moody sampled orchestra, parallax depth | Mode 7 reserved for three moments: the Great Seal press, the Retention alarm spin in Scene 2, the title logo zoom |
| Donkey Kong Country (Rare, 1994) | Pre-rendered-looking sprites on a tight palette, deep parallax, David Wise's ambient pads soaked in echo | model-painted art cleaned to SNES palettes; 2-3 parallax layers per area; echo-heavy pads in Stage 2 |
| Chrono Trigger (Square, 1995) | Melody first; small instrument set used expressively; motifs carried across the whole score | one CorporateWave melody remixed per cue (the NES theme in `docs/THEME.md` carries over) |
...
| --- | --- | --- | --- | --- |
| D-pad | arrows or WASD | d-pad or left stick | walk | walk, crouch, aim (hold R) |
| Y | Z or J | X / left face | punch (chain three); grab a reeling foe | cast |
| B | X or K | A / bottom face | jump (Y in air = jump kick) | jump |
| A | C or L | B / right face | Emergency Injunction (full meter) | Emergency Injunction |
| X | V or I | Y / top face | — | swap enchantment |
| L / R | Q / E | shoulders | evasive step back / forward | R held: stand and aim in eight directions |
...
The NES mapping stays for `?famicom`. `src/input.mjs` answers the pad for the active profile.
