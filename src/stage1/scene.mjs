...
const MS = 1000 / 60;
const SOUND = { punch: 'punch', hit: 'hit', heavy: 'knockdown', jump: 'jump', land: 'land', grab: 'grab', throw: 'throw', step: 'step' };
...
export class Stage1Scene extends Phaser.Scene {
  ...
  draw() {
    ...
    const sprites = things.flatMap(({ f, o }) => {
    ...
    if (this.freezeAt !== null) {
      const foes = w.fighters.filter((f) => f.team === 'foe').map((f) => `${f.dummy ? 'DUM' : 'FOE'} ${f.state.slice(0, 4).toUpperCase()} ${f.hp}`);
