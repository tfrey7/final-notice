...
// a car to stop before boarding and riding it up. It answers the buttons held this frame. Pure.
...
function standingOn(p) {
  ...
  return SHAFT.ledges.find((l) => l.row * TILE === p.y && p.x + p.w / 2 > l.c0 * TILE && p.x - p.w / 2 < (l.c1 + 1) * TILE) ?? null;
}
...
export function botButtons(bot, s) {
  ...
  const here = s.hang ?? (s.ride && SHAFT.cars[s.ride.k]) ?? standingOn(p);
  if (here) {
    bot.i = SHAFT.route.indexOf(here);
    bot.steer = 0;
  }
  const next = SHAFT.route[bot.i + 1];
  if (!next) return down;
  const hw = p.w / 2;

  if (!here) {
    if (bot.hold-- > 0) down.add('b');
    const target = next.kind === 'ledge' ? next : SHAFT.route[bot.i + 2];
