...
export function drawRouteMap(g, fill, p, tune, { x, y }) {
  ...
  rows.forEach((r, i) => {
    ...
    r.keys.forEach((k, j) => {
      ...
      const bw = keyW(k);
      const colour = !r.on ? 0x2a2a2e : lit ? (k === 'X' ? 0x4070e0 : 0x40c060) : 0x4a4a52;
      g.fillStyle(colour).fillRect(cx, ry, bw, 10);
      drawString(fill, k, cx + (bw - measure(k)) / 2, ry + 1, lit ? rgb15(31, 31, 31) : rgb15(20, 20, 22), null);
      cx += bw + 2;
    });
